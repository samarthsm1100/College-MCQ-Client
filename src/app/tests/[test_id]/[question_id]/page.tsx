...
export default function QuestionsPage({params}:{params:{question_id:string}}) {
  ...
  const {setOption,toggleReview,toggleSave} = useResults();

  const results = JSON.parse(window.localStorage.getItem('results') as string);
  const index = (question?.id as number)-1;

  const [radio, setRadio] = React.useState(results ? results[index].option.toString() : ''); 
  ...
  return (
    ...
    <RadioGroup className='mt-4' value={radio} onValueChange={setRadio}>
      <Radio className='transition-all duration-600 py-4 my-2 min-w-44 focus-within:border-blue-500 border-gray-300 border-2 rounded-md'  value={question?.options[0].id.toString()??''}>{question?.options[0].option}</Radio>
      <Radio className='transition-all duration-600 py-4 my-2 min-w-44 border-gray-300 border-2 focus-within:border-blue-500 rounded-md'  value={question?.options[1].id.toString()??''}>{question?.options[1].option}</Radio>
    ...
    <Button 
      ...
      onClick={() => {
        setOption(index,parseInt(radio));
        toggleSave(index);
      }}
    >
      Save
